...
class ZooKeeper {
public:
    ZooKeeper(const std::string& server_hosts, ZooWatcher* global_watcher =
            nullptr, int timeout_ms = TIME_OUT_MS);
    ...
    std::string RecursiveCreate(const std::string& path,
            const std::string& value = std::string("123"),
            int flag = 0);
