...
void ZooKeeper::GlobalWatchFunc(zhandle_t* h, int type, int state,
...
ZooKeeper::ZooKeeper(const std::string& server_hosts,
        ZooWatcher* global_watcher, int timeout_ms) :
        global_watcher_(global_watcher) {
    set_default_debug_level();

    server_hosts_ = server_hosts;
    zoo_handle_ = zookeeper_init(server_hosts.c_str(), GlobalWatchFunc,
            timeout_ms, nullptr, // client id
            this, 0);
    if (!zoo_handle_) {
...
void ZooKeeper::reconnect() {
	if (zoo_handle_) {
		zookeeper_close(zoo_handle_);
	}

	zoo_handle_ = zookeeper_init(server_hosts_.c_str(), GlobalWatchFunc,
			TIME_OUT_MS, nullptr, this, 0);
}
...
std::string ZooKeeper::Create(const std::string& path, const std::string& value,
        int flag) {
    std::string path_buffer;
    path_buffer.resize(path.size() + 64);

    auto zoo_code = zoo_create(zoo_handle_, path.c_str(), value.data(),
            value.size(), &ZOO_OPEN_ACL_UNSAFE, flag,
            const_cast<char*>(path_buffer.data()), path_buffer.size());
...
std::string ZooKeeper::CreateIfNotExists(const std::string& path,
...
std::string ZooKeeper::RecursiveCreate(const std::string& path,
        ...
        int flag)
{
      ...
      return CreateIfNotExists(path, value, flag);
}
...
std::string ZooKeeper::Get(const std::string& path, bool watch) {
    ...
    value_buffer.resize(node_stat.dataLength + 10);

    int buffer_len = value_buffer.size();
    auto zoo_code = zoo_get(zoo_handle_, path.c_str(), watch,
            const_cast<char*>(value_buffer.data()), &buffer_len, &node_stat);
...
void ZooKeeper::Set(const std::string& path, const std::string& value) {
    ...
    auto zoo_code = zoo_set(zoo_handle_, path.c_str(), value.data(),
            value.size(), node_stat.version);
    CHECK_ZOOCODE_AND_THROW(zoo_code)
}
...
std::vector<std::string> ZooKeeper::GetChildren(const std::string& parent_path,
    ...
    auto zoo_code = zoo_get_children(zoo_handle_, parent_path.c_str(), watch,
            &child_vec);
    ...
    deallocate_String_vector(&child_vec);
